...
$MagicParser_copyright = "Copyright (c) 2005 IAAI Software, All Rights Reserved";
...
function MagicParser_xml_endTag($parser, $name) {
    ...
    if (($MagicParser_xml_path == $MagicParser_xml_record_path) && !$MagicParser_xml_done) {
        if (is_object($handler_class)) {
            $MagicParser_xml_done = call_user_func(array($handler_class, $MagicParser_record_handler), $MagicParser_xml_current_record);
        } else
...
function MagicParser_parse($filename, $record_handler, $format_string = "") {
    ...
    if (is_array($record_handler)) {
        MagicParser_set_handler_class($record_handler[0]);
        $MagicParser_record_handler = $record_handler[1];
    }
    else $MagicParser_record_handler = $record_handler;
...
function MagicParser_set_handler_class($hc) {
    global $handler_class;
    $handler_class = $hc;
}
